...
public:
    ...
    T Pop()
    {
        ...
        SinkDown(0);
        ValueToIndexMap.Remove(MinValue);
        return MinValue;
    }
...
private:
    ...
    {
        int32 LeftChild, RightChild, MinChild;
        while (true)
        {
            LeftChild = Index * 2 + 1;
            ...
            if (LeftChild < Heap.Num() && Heap[LeftChild] < Heap[MinChild])
            {
                MinChild = LeftChild;
            }

            if (RightChild < Heap.Num() && Heap[RightChild] < Heap[MinChild])
            {
                MinChild = RightChild;
            }
